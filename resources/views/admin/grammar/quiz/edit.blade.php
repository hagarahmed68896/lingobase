...
@section('title', 'Edit Quiz Question - ' . $lesson->title)
...
<div class="table-container" style="border: none; background: transparent; box-shadow: none; overflow: visible;">
    <div style="background: white; padding: 2.5rem; border-radius: 1.5rem; border: 1px solid #e5e7eb; margin-bottom: 2rem; box-shadow: 0 10px 30px -10px rgba(0,0,0,0.05); max-width: 800px; margin: 0 auto;">
        <div style="margin-bottom: 2rem;">
            <h1 style="font-size: 1.75rem; font-weight: 800; color: #111827; margin: 0;">Edit Question</h1>
            <p style="color: #6b7280; font-size: 0.95rem; margin-top: 0.25rem;">For lesson: {{ $lesson->title }}</p>
        </div>

        <form action="{{ route('admin.grammar.quiz.update', $question) }}" method="POST" id="question-form">
            @csrf
            @method('PUT')
            ...
            <div style="margin-bottom: 2rem;">
                ...
                <textarea name="question" class="search-input" style="width: 100%; height: 100px; border-radius: 0.75rem; padding: 1rem; resize: none;" placeholder="Enter the question here..." required>{{ old('question', $question->question) }}</textarea>
            </div>
                ...
                <div id="options-container" style="display: grid; gap: 1rem;">
                    @foreach($question->options as $index => $option)
                        <div class="option-row" style="display: flex; align-items: center; gap: 1rem; background: #f8fafc; padding: 1rem; border-radius: 1rem; border: 1px solid #e2e8f0;">
                             ...
                             <input type="radio" name="correct_option" value="{{ $index }}" {{ $option->is_correct ? 'checked' : '' }} required style="width: 20px; height: 20px; accent-color: var(--primary);">
                             <input type="text" name="options[{{ $index }}][text]" value="{{ $option->option_text }}" class="search-input" style="flex: 1; border-radius: 0.5rem;" placeholder="Option {{ $index + 1 }}" required>
                             @if($index > 0)
            ...
            <div style="display: flex; gap: 1rem; justify-content: flex-end; padding-top: 2rem; border-top: 1px solid #e2e8f0;">
                ...
                <button type="submit" class="btn btn-primary" style="padding-left: 2.5rem; padding-right: 2.5rem;">Update Question</button>
            </div>
        </form>
    </div>
</div>
