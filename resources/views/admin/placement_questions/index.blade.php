...
@section('title', request('language_id') ? 'Manage ' . ($languages->firstWhere('id', request('language_id'))->name ?? '') . ' Placement Test' : 'Manage Placement Tests')
    ...
    <div style="background: white; padding: 2.5rem; border-radius: 1.5rem; border: 1px solid #e5e7eb; margin-bottom: 2rem; box-shadow: 0 10px 30px -10px rgba(0,0,0,0.05); position: relative; overflow: hidden;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 2rem; flex-wrap: wrap; gap: 1.5rem; position: relative; z-index: 1;">
            <div>
                ...
                <h1 style="font-size: 2.25rem; font-weight: 800; color: #111827; margin: 0; line-height: 1.2;">{{ request('language_id') ? ($languages->firstWhere('id', request('language_id'))->name ?? '') . ' Placement Questions' : 'All Placement Questions' }}</h1>
            ...
            <div style="display: flex; gap: 1rem;">
                ...
                <a href="{{ route('admin.placement-questions.create', request('language_id') ? ['language_id' => request('language_id')] : []) }}" class="btn btn-primary" style="padding: 0.875rem 1.75rem; border-radius: 1rem; display: flex; align-items: center; gap: 0.75rem; font-weight: 700; font-size: 0.95rem; box-shadow: 0 4px 14px 0 rgba(0, 145, 80, 0.25); transition: all 0.3s ease;">
